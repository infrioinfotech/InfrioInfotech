...
const techIcons: Record<string, string> = {
  ...
  NLP: "https://cdn-icons-png.flaticon.com/512/1256/1256650.png", // Fallback for generic
};

export function PortfolioCard({ title, category, description, image, technologies, index }: PortfolioCardProps) {
  return (
    <motion.div
      ...
      viewport={{ once: true }}
      className="group relative overflow-hidden rounded-3xl bg-black border border-white/10 flex flex-col lg:flex-row shadow-2xl hover:shadow-[#e11d48]/20 transition-all duration-500"
    >
      {/* Content Section */}
        ...
        {technologies && (
          <div className="flex items-center gap-4 mb-8">
            {technologies.map((tech) => (
              <div 
                key={tech} 
                className="w-10 h-10 rounded-full bg-white/5 border border-white/10 flex items-center justify-center p-2 tooltip"
                title={tech}
              >
                {techIcons[tech] ? (
